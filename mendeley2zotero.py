...
sp = parser.add_subparsers(
    help="", dest='command')
sp_added = sp.add_parser(
    'added_dates',
    help='Update \"Date Added\" field',
    description='Updates the Zotero field \"Date Added\" from Mendeley data')
sp_repair = sp.add_parser(
    'repair_dates',
    help='Repair \"Date Added\" field',
    description='Repairs the Zotero field \"Date Added\" to proper format')
sp_collections = sp.add_parser(
    'collections',
    help='Update collections',
    description='Updates the document associations to existing collections (for now create the desired collections by hand before)')
parser.add_argument(
    "-z", "--zotero_db",
    action="store",
    default="", help="zotero sqlite database path", required=True)
parser.add_argument(
    "-m", "--mendeley_db", action="store",
    default="", help="mendeley sqlite database path", required=True)
...
if args.command == "repair_dates":
    ...
    # get all zotero items by getting their titles (workaround for possibly
    # incomplete reverse engineered db classes)
    field_title = session["z"].query(db_class["z"].Field).filter(
        db_class["z"].Field.fieldName == "title").one()
    type_attachment = session["z"].query(db_class["z"].Itemtype).filter(
        db_class["z"].Itemtype.typeName == "attachment").one()
    z_titles = session["z"].query(db_class["z"].Itemdatum).filter(
        db_class["z"].Itemdatum.fieldID == field_title.fieldID).all()
    ...
    for i, z_title_ob in enumerate(z_titles):
        ...
        m_titles = session["m"].query(db_class["m"].Document).filter(
            db_class["m"].Document.title == z_title).all()
        ...
        if args.command == "added_dates":
            # get the maximum date added (can have more than one title
            # matching)
            ...
            if i % 10 == 0 or i == len(z_titles) - 1:
                try:
                    ...
                    print("Error during batch commit. Rolling back. %s" % e)
                    session.rollback()
                print("[%i/%i] " % (i + 1, len(z_titles))),
                print(" batch committed.")
        ...
        if args.command == "collections":
            ...
            for m_object in m_titles:
                ...
                folders_assoc = session["m"].query(db_class["m"].Documentfolder).filter(
                    db_class["m"].Documentfolder.documentId == m_object.id).all()

                for f in folders_assoc:
                    folder = session["m"].query(db_class["m"].Folder).filter(
                        db_class["m"].Folder.id == f.folderId).one()
                    # get matching zotero collections
                    collections = session["z"].query(db_class["z"].Collection).filter(
                        db_class["z"].Collection.collectionName == folder.name).all()
